...
if __name__ == '__main__':
    ...
    # this should contain paths to the output files that are produced when
    # running invert_etas.py this information is then read and processed
    # below to simulate a continuation of the catalog that was the input of
    # the inversion
    ...
    print("using parameters calculated on",
          parameters_dict["calculation_date"], "\n")
    ...
    assert len(catalog) == len(sources), \
        ...
        f"{str(len(catalog))} -- {str(len(sources))}"
    assert catalog.magnitude.min() == m_ref, \
        f"smallest magnitude in sources is {str(catalog.magnitude.min())}" \
        f" but I am supposed to simulate above {str(m_ref)}"

    # background rates
    ip = pd.read_csv(simulation_config["fn_ip"], index_col=0)
    ip.query("magnitude>=@m_ref -@delta_m/2", inplace=True)
    ip = gpd.GeoDataFrame(
        ip, geometry=gpd.points_from_xy(
    ...
    print(
        "m ref:",
        m_ref,
        "min magnitude in training catalog:",
    ...
    continuation.query(
        "time>=@forecast_start_date and time<=@forecast_end_date "
